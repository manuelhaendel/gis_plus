...
In the case of the *Circle* and *Wedge* neighborhood, before iterating through the raster cells, the raster gets padded with a number of rows and columns. The number depends on the radius the user determines and ensures that the window around a cell close to the edge of the input raster never extends past the boundary of the padded raster. The cells of the padding are filled with *NA* values and hence the *ignore_nodata* argument defines how the edge cells will be filled. When *ignore_nodata* is set to *true* (the default), the statistic will be applied to all the values of the window that are not *NA*, when it is set to *false* the cell will be replaced by *NA*, effectively reducing the size of the output raster.  

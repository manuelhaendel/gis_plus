...
For the square and the rectangle neighborhood, we use two functions: *rectangfun* and *get_newdata*. The first determines the number of pixels from the central pixel to all four sides. This dependents on whether the height and width of the window is even or odd. Is the width even, the central pixel is assigned to the left pixel of the two possible middle pixels. For an even height, the central pixel is the top pixel.
...
<!-- Der folgende Satz ist ein bisschen lang und unübersichtlich, vielleicht kann man den nochmal teilen? -->
In the case of the *Circle* and *Wedge* neighborhood, before iterating through the raster cells the raster gets padded with a number of rows and columns, which depend on the radius the user determines and ensures that the window around the cell close to the edge of the input raster never extends past the boundary of the padded raster. The cells of the padding are filled with *nodata* values and hence the *ignore_nodata* argument defines how the edge cells will be filled. When *ignore_nodata* is set to *true* (the default), the statistic will be applied to all the values of the window that are not *na*, when it is set to *false* the cell will be replaced by *nodata*, effectively reducing the size of the output raster.
<!-- Ich glaub ich würd mich auf na oder nodata festlegen und nicht beides in einem Satz verwenden für das Gleiche. Grad weils schief geschrieben ist wirkt das so, als wärs nen festgelegter Begriff und da verwirrt es bisschen. -->
